...
// Helper kecil di atas file atau tepat sebelum pemakaian:
const formatYmdLocal = (d, timeZone = "Asia/Makassar") =>
...
const generateTransactionCode = async () => {
  const date = new Date();
  const dateYmd = formatYmdLocal(date, "Asia/Makassar"); // contoh: "2025-06-18"
  const dateStr = dateYmd.replace(/-/g, ""); // "20250618"
  ...
  if (result.length > 0 && result[0].transaction_code) {
    ...
    const lastNumber = parseInt(
      result[0].transaction_code.split("-").pop(),
      10
    );
...
const createTransaction = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      (!debt_details || !debt_details.customer_id)
    ) {
      return res
        .status(400)
        .json({
    ...
    for (const item of items) {
      ...
      if (item.variant_id) {
        ...
        if (variants.length === 0) {
          throw new Error(
            `Varian produk dengan ID ${item.variant_id} tidak ditemukan atau tidak aktif!`
          );
        }
        const variant = variants[0];

        if (
          variant.item_type === "barang" &&
          variant.current_stock < item.quantity
        ) {
          throw new Error(
            `Stok untuk "${variant.item_name} - ${variant.variant_name}" tidak cukup! Sisa: ${variant.current_stock}`
          );
        }
        ...
        if (product.has_variants) {
          throw new Error(
            `Produk "${product.item_name}" memiliki varian. Silakan pilih salah satu varian.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `Stok untuk "${product.item_name}" tidak cukup! Sisa: ${product.current_stock}`
          );
        }
    ...
    if (
      payment_method === "cash" &&
      payment_received < server_calculated_total
    ) {
...
const getTransactionById = async (req, res) => {
  try {
    ...
    if (
      req.user.role === "admin" &&
      transactions[0].admin_id !== req.user.user_id
    ) {
...
const getDailySummary = async (req, res) => {
  try {
    ...
    const targetDate = date || formatYmdLocal(new Date(), 'Asia/Makassar');
