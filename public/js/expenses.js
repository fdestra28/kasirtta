...
async function initExpenses() {
  ...
  const ymdLocal = (d, tz = "Asia/Makassar") =>
  ...
  document.getElementById("expenseFilterDateStart").value = ymdLocal(
    thirtyDaysAgo,
    "Asia/Makassar"
  );
  document.getElementById("expenseFilterDateEnd").value = ymdLocal(
    today,
    "Asia/Makassar"
  );
...
function setupExpensePageEvents() {
  ...
  document
    .getElementById("filterExpenseBtn")
  ...
  document
    .getElementById("cancelExpenseModal")
    .addEventListener("click", () => closeModal("expenseModal"));
...
function renderExpenseTable() {
  ...
  if (expenseList.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--color-text-muted);">Tidak ada data pengeluaran pada periode ini.</td></tr>';
  } else {
...
function updateExpensePagination() {
  ...
  document.getElementById(
    "expensePageInfo"
  ).textContent = `Halaman ${expenseCurrentPage}`;
  document.getElementById("expensePrevBtn").disabled = expenseCurrentPage === 1;
  document.getElementById("expenseNextBtn").disabled =
    expenseCurrentPage * expenseLimit >= totalItems;
}
...
async function deleteExpense(expenseId) {
  ...
  if (
    !confirm(
      "Apakah Anda yakin ingin menghapus data pengeluaran ini? Aksi ini tidak dapat dibatalkan."
    )
  ) {
    return;
  }
