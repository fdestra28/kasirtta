...
function initializeUI() {
  ...
  sidebarCollapsed =
    savedSidebarState !== null ? savedSidebarState === "true" : true;
...
function updateUserInterface() {
  document.getElementById(
    "userInfo"
  ).textContent = `${currentUser.full_name} (${currentUser.role})`;
...
function openModal(modalId) {
  ...
  if (modal) {
    ...
    setTimeout(() => {
      // [FIX] Tambahkan pengecekan sebelum mengakses .style
      const backdrop = modal.querySelector(".modal-backdrop");
      if (backdrop) {
...
function setupEventListeners() {
  ...
  if (connectPrinterBtn) {
    connectPrinterBtn.addEventListener("click", () => {
      // Jika sudah terhubung, tombol ini berfungsi sebagai disconnect
...
async function loadDashboard() {
  try {
    ...
    if (summaryData.success) {
      ...
      const topProductsHtml = summaryData.data.top_products
        .slice(0, 5)
        .map(
          (p, index) => `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; opacity: 0; transform: translateX(-20px); animation: slideInFromLeft 0.5s ease ${
                  index * 0.1
                }s forwards;">
      ...
      document.getElementById("topProducts").innerHTML =
        topProductsHtml || '<p class="text-muted">Belum ada data</p>';
    }
    ...
    // Render ke HTML
    const lowStockHtml = allLowStockItems
      .slice(0, 5)
      .map(
        (item, index) => `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; opacity: 0; transform: translateX(-20px); animation: slideInFromLeft 0.5s ease ${
    ...
    document.getElementById("lowStock").innerHTML =
      lowStockHtml || '<p class="text-muted">Stok aman</p>';
...
async function generateReport() {
  ...
  try {
    ...
    const response = await apiRequest(
      `/transactions/report/data?start_date=${startDate}&end_date=${endDate}&v=${cacheBuster}`
    );
    ...
    if (result && result.success && result.data) {
      console.log(
        "API Sukses. Data (result.data):",
        JSON.stringify(result.data, null, 2)
      );
      console.log(
        "Expense Summary dari API (result.data.expense_summary):",
        JSON.stringify(result.data.expense_summary, null, 2)
      );
      ...
      document.getElementById("reportContent").innerHTML =
        '<p class="text-danger">Gagal memuat data laporan atau data tidak lengkap.</p>';
      document.getElementById("reportChartsContainer").innerHTML = "";
    }
  } catch (error) {
    console.error("Generate report error:", error); // <-- LOG ERROR di CATCH
    document.getElementById("reportContent").innerHTML =
      '<p class="text-danger">Terjadi kesalahan saat memuat laporan</p>';
...
function renderReportCharts(reportData) {
  ...
  if (reportData.revenue_trend && reportData.revenue_trend.length > 0) {
    revenueTrendChartInstance = new Chart(revenueCtx, {
      ...
      type: "line",
      data: {
        labels: reportData.revenue_trend.map((item) =>
          formatDateShort(item.period)
        ),
        datasets: [
          {
            label: "Pendapatan",
            data: reportData.revenue_trend.map((item) =>
    ...
    revenueCtx.canvas.parentNode.innerHTML =
      '<p style="text-align: center; padding-top: 50px; color: var(--color-text-muted);">Tidak ada data pendapatan untuk periode ini.</p>';
  }

  // --- Render Expense Composition Chart ---
  const expenseCtx = document
    .getElementById("expenseCompositionChart")
    .getContext("2d");
  const expenseSummaryForChart = reportData.expense_summary;

  if (
    expenseSummaryForChart &&
    expenseSummaryForChart.by_category &&
    expenseSummaryForChart.by_category.length > 0
  ) {
    const categories = expenseSummaryForChart.by_category;
    const totalCategories = categories.length;
    const dynamicColors = categories.map((_, index) =>
      generateHslColor(index, totalCategories)
    );

    expenseCompositionChartInstance = new Chart(expenseCtx, {
      // Simpan instance ke variabel global
      type: "doughnut",
      data: {
    ...
    expenseCtx.canvas.parentNode.innerHTML =
      '<p style="text-align: center; padding-top: 50px; color: var(--color-text-muted);">Tidak ada data pengeluaran untuk periode ini.</p>';
  }
}
...
function renderReportTables(reportData) {
  console.log(
    "MEMULAI renderReportTables. Menerima reportData:",
    JSON.stringify(reportData, null, 2)
  ); // <-- LOG AWAL FUNGSI
  const { product_performance, cashier_performance, expense_summary } =
    reportData;
  const totalRevenue = reportData.revenue_trend.reduce(
    (sum, item) => sum + parseFloat(item.total_revenue),
    0
  );
  const totalTransactions = reportData.revenue_trend.reduce(
    (sum, item) => sum + parseInt(item.total_transactions),
    0
  );
  console.log(
    "Expense Summary untuk tabel (expense_summary):",
    JSON.stringify(expense_summary, null, 2)
  );
  const totalExpense =
    expense_summary && expense_summary.summary
      ? parseFloat(expense_summary.summary.total_expense) || 0
      : 0;
        ...
        <div class="report-container glass-card" id="mainReportContainer">
            <div class="report-summary">
                <div class="summary-card">
                    ...
                    <p class="summary-value text-success">${formatCurrency(
                      totalRevenue
                    )}</p>
                </div>
                <div class="summary-card">
                    <h4>Total Pengeluaran</h4>
                    <p class="summary-value text-danger">${formatCurrency(
                      totalExpense
                    )}</p>
                </div>
                <div class="summary-card">
...
function exportToExcel(tableContainerId, filename) {
  ...
  let csvContent = `"${
    container.querySelector("h1, h2, h3")?.textContent || "Laporan"
  }"\n`;
  const startDate = document.getElementById("startDate")?.value;
  const endDate = document.getElementById("endDate")?.value;
  ...
  tables.forEach((table) => {
    ...
    if (
      sectionHeader &&
      (sectionHeader.tagName === "H3" || sectionHeader.tagName === "H4")
    ) {
...
function initHistory() {
  ...
  if (!filterBtn._listener) {
    ...
    document
      .querySelector("#historyDetailModal .close")
      .addEventListener("click", () => {
        closeModal("historyDetailModal");
      });

    document
      .getElementById("historyCloseDetailBtn")
      .addEventListener("click", () => {
        closeModal("historyDetailModal");
      });
...
async function loadHistory(page) {
  ...
  try {
    const response = await apiRequest(
      `/transactions?start_date=${startDate}&end_date=${endDate}&limit=${historyLimit}&offset=${offset}`
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      tbody.innerHTML =
        '<tr><td colspan="5" class="text-danger">Gagal memuat riwayat.</td></tr>';
    }
  } catch (error) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-danger">Terjadi kesalahan koneksi.</td></tr>';
  }
}
  ...
  if (!initStock.initialized) {
    ...
    document.querySelectorAll("#stockFilterTabs .filter-tab").forEach((tab) => {
      tab.addEventListener("click", (e) => {
        ...
        stockCurrentPage = 1;
        document
          .querySelectorAll("#stockFilterTabs .filter-tab")
    ...
    let searchTimeout;
    document
      .getElementById("stockFilterSearch")
    ...
    document
      ...
      .addEventListener("submit", updateStock);
    document
      .getElementById("cancelStockModal")
      .addEventListener("click", () => closeModal("stockModal"));
...
function openStockModal(productId, variantId, itemName) {
  ...
  document.querySelector(
    "#stockModal h3"
  ).textContent = `Update Stok: ${itemName}`;
  ...
  if (paginatedItems.length === 0) {
    ...
    tbody.innerHTML = paginatedItems
      .map((item, index) => {
        ...
        return `
                <tr style="opacity: 0; transform: translateY(10px); animation: fadeInUp 0.3s ease ${
                  index * 0.05
                }s forwards;">
                    ...
                    <td>${statusBadge}</td>
                    <td>
                        <button class="btn btn-sm" onclick="openStockModal(${
                          item.product_id
                        }, ${variantIdParam}, '${item.item_name.replace(
          /'/g,
          "\\'"
        )}')">
...
function updateStockPagination(totalItems) {
  document.getElementById(
    "stockPageInfo"
  ).textContent = `Halaman ${stockCurrentPage}`;
  document.getElementById("stockPrevBtn").disabled = stockCurrentPage === 1;
  document.getElementById("stockNextBtn").disabled =
    stockCurrentPage * stockLimit >= totalItems;
}
...
function renderHistoryTable(transactions) {
  ...
  if (transactions.length === 0 && historyCurrentPage === 1) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align:center; padding: 20px;">Tidak ada transaksi pada periode ini.</td></tr>';
    return;
  }
      ...
      (t, index) => `
        <tr style="opacity: 0; transform: translateY(10px); animation: fadeInUp 0.3s ease ${
          index * 0.05
            ...
            <td>
                <button class="btn btn-sm" onclick="viewTransactionDetail(${
                  t.transaction_id
                })">
                    <ion-icon name="eye-outline"></ion-icon>
                    Detail
                </button>
                <button class="btn btn-sm btn-primary" onclick="reprintReceipt(${
                  t.transaction_id
                })">
                    <ion-icon name="print-outline"></ion-icon>
                    Cetak
...
async function viewTransactionDetail(transactionId) {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.getElementById(
        "historyDetailTitle"
      ).textContent = `Detail Transaksi ${trx.transaction_code}`;
      ...
      document.getElementById("historyReprintBtn").onclick = () =>
...
// Ganti fungsi reprintReceipt dengan ini
async function reprintReceipt(transactionId) {
    try {
        ...
        if (data.success) {
            const trxData = { ...data.data, items: data.data.details };
            // Langsung panggil fungsi print yang baru
            await thermalPrinter.print(trxData, appSettings);
        } else {
  ...
  const receiptHTML = `
        ...
        <body>
            <div class="header">
                ...
                <p>${appSettings.store_address || ""}<br>${
    appSettings.store_phone || ""
  }</p>
            </div>
            <div class="info">
            ...
            <div class="total">
                <div class="item"><span>Total</span><span>${formatCurrency(
                  transaction.total_amount
                )}</span></div>
                <div class="item"><span>Bayar</span><span>${formatCurrency(
                  transaction.payment_received
                )}</span></div>
                <div class="item"><span>Kembali</span><span>${formatCurrency(
...
function handleGlobalShortcuts(e) {
  ...
  if (e.altKey) {
    ...
    switch (e.key.toLowerCase()) {
      ...
      case "h":
        ...
        break;
      case "p":
        if (currentUser.role === "owner") navigateToPage("products");
        break;
      case "r":
        if (currentUser.role === "owner") navigateToPage("reports");
        break;
      case "u":
        if (currentUser.role === "owner") navigateToPage("users");
        break;
    }
    ...
    switch (e.key.toLowerCase()) {
      case "f":
        ...
        const searchInput = document.querySelector(
          '.page.active input[type="text"][placeholder*="Cari"]'
        );
        if (searchInput) {
          searchInput.focus();
          searchInput.select();
      ...
      case "a":
        if (currentPage === "cashier") {
          e.preventDefault();
          const firstQtyInput = document.querySelector(
  ...
  switch (e.key) {
    case "F3":
      e.preventDefault();
      showNotification(
        "Shortcut F3 (Cari Berikutnya) belum diimplementasikan.",
        "info"
      );
      break;
    ...
    case "Escape":
      ...
      const searchInputEl = document.querySelector(
        '.page.active input[type="text"][placeholder*="Cari"]:focus'
      );
...
async function reprintLastTransaction() {
  try {
    ...
    if (data.success && data.data.length > 0) {
      ...
      showNotification(
        `Mencetak ulang transaksi terakhir: ${data.data[0].transaction_code}`,
        "success"
      );
    } else {
      showNotification(
        "Tidak ada transaksi terakhir yang bisa dicetak ulang.",
        "warning"
      );
    }
  } catch (error) {
